...
- [ ] **VRMボーンの可視化と操作** ✨
  - [ ] VRMヒューマノイドボーン構造の解析
    - [ ] @pixiv/three-vrm APIを使用したボーン階層へのアクセス
    - [ ] VRMHumanoid.rawBonesのマッピングと整理
    - [ ] 主要ボーン（頭、首、腕、手など）の特定と分類
  - [ ] ボーン階層の視覚化
    - [ ] SkeletonHelperによる全体骨格の表示
    - [ ] 主要ボーンポイントの視覚化（小さな球体オブジェクト）
    - [ ] 選択可能なボーン表示/非表示切り替え機能
    - [ ] 骨格表示の色・透明度設定
  - [ ] ボーン選択システム
    - [ ] Raycastingによる3Dビュー上でのボーン選択
    - [ ] 選択したボーンのハイライト表示
    - [ ] ボーンリストUIからの選択機能
- [ ] **TransformControlsとの統合** ✨
  - [ ] 選択されたボーンへのTransformControlsアタッチ
  - [ ] 回転モード（Rotate）の実装
    - [ ] ローカル/ワールド座標系の切り替え
    - [ ] 回転制限の実装（人体の自然な可動域）
  - [ ] 移動モード（Position）の実装（オプション）
  - [ ] UI上での操作モード切り替え
- [ ] **ボーン回転の実装** ✨
  - [ ] クォータニオンを使用した回転操作
  - [ ] 親ボーンから子ボーンへの階層的変換の伝播
  - [ ] 回転角度の保存と管理
  - [ ] IK制約との連携準備（将来拡張用）
- [ ] **ポーズのリセット機能** ✨
  - [ ] 初期姿勢（T-pose）への復元機能
...
1. **VRMボーンの可視化と操作**
   - VRMヒューマノイドボーン構造の解析
   - ボーン階層の視覚化（SkeletonHelper）
   - 主要ボーンの選択機能実装
   - TransformControlsによる回転操作
